...
- **Assumption:** Placeholder icons are generated by `scripts/generate-icons.ts` (hand-rolled PNG encoder, no deps; `bun run icons`) — solid `#0b1220` background with blocky "CW" in `#7ea6ff`; 192, 512, and a maskable 512 with a wider safe zone. Committed as files in `public/icons/`; real artwork is an explicit later item. Generator deleted during review — see Finding 3 in "Review-fix log (docs + tooling)" below (supersedes the generation note; the committed `public/icons/` files stay).
...
- **Finding 3 (gate wiring lacks component-level test) — applied (test written).** Initially
  ...
  verifies the gate wiring end-to-end. See the Sub-phase 5 log below for the seam and config
  judgment calls this required.

No findings were rejected.

## Sub-phase 5 — App-gate component test (2026-07-12)
...
- **Browser projects now load the svelte plugin.** The Sub-phase 3 assumption "browser projects
...
- **Verified:** `bun run test:browser` green (6 tests: 2 pre-existing OPFS + 4 new gate);
  ...
  (Sub-phase 3 note) — but it now has the same plugin/define setup so it is not left broken.
...
- **Finding 1 (GPU-golden flag naming) — applied.** testing.md's GPU-golden "Where" line
  ...
  returns a null adapter (see "Sub-phase 3 → Exact WebGPU launch flags" above); the working set is
  the Vulkan-on-SwiftShader trio. Softened to "SwiftShader; the exact Chromium flags live in
  `vitest.config.ts`". Supersedes the earlier Sub-phase 4 note that testing.md names that flag
  "quoting the plan".
- **Finding 2 (E2E tolerance stated as settled) — applied.** `±1 frame` is anchored in
  ...
  (Phase 3/4). Matches the Sub-phase 4 open question recorded above.
- **Finding 3 (icon generator is one-shot dead code) — applied (deleted).** Removed
  ...
  later item (Sub-phase 2 assumption above). The generator had no ongoing role — not in
...
- **Capability gate policy (Finding 1 / product decision).** A review pass briefly added a
  per-capability `required` flag to `capabilities.ts` (all four `true`) as an explicit seam for
  graceful degradation; the user then had it removed as an inert speculative seam, since
  product.md "Platform requirements" and ADR 0002 make all four capabilities (WebGPU, camera,
  OPFS, speech) hard requirements today — the gate is plain "every probe passes". The underlying
  product question stays open: should a missing/failed `speechSynthesis` (or camera, on a
  review-only desktop that just reads sessions) degrade gracefully instead of showing the
