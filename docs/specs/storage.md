...
OPFS is origin-scoped browser storage: clearing site data deletes everything. The app requests `navigator.storage.persist()` once, after the first successful write (asking before any data exists would waste the prompt), and surfaces whether it was granted. Regular exports are the user's backup; the UI should gently prompt after sessions if no recent export exists.
